...
/** \brief valida que la cadena ingresada cumpla el formato patente (AAA-NNN)
 *
 * \param str[] char
 * \return int true=1 false=(0=caracetes diferente de 7, -1 primeros 3 char no son letras, -2 char medio no '-', -3 ultimos 3 char no numeros)
 *
 */
int validPatent(char str[]);

/** \brief Solicita ingresar al usuario una patente y la valida con el formato (AAA-NNN). Si es valida la copia a la cadena patent
 *
 * \param message[] char
 * \param patent[] char
 * \return void
 *
 */
void inputPatent(char message[], char patent[]);
...
/** \brief Recive una fecha con valores numericos y retorna una cadena
 *
 * \param str[] char dd/mm/yyy
 * \param d int
 * \param m int
 * \param y int
 * \return void
 *
 */
void setDateFormat(char str[], int d, int m, int y);

/** \brief Recive una fecha con valores numericos y retorna una cadena formato DB SQL
 *
 * \param str[] char yyyymmdd
 * \param d int
 * \param m int
 * \param y int
 * \return void
 *
 */
void setDateFormat4Sort(char str[], int d, int m, int y);
